...
export function PopularTopics({ onSelect, disabled }: PopularTopicsProps) {
  return (
    <div className="flex flex-col gap-3 max-sm:min-w-0 sm:flex-row sm:items-center">
      <p className="shrink-0 text-xs font-semibold uppercase tracking-[0.14em] text-brand-gray">
        Popular now
      </p>
      <div className="scrollbar-hide flex flex-wrap gap-2 max-sm:-mx-3 max-sm:flex-nowrap max-sm:overflow-x-auto max-sm:px-3 max-sm:pb-1">
        {TOPICS.map((topic) => (
          <button
            key={topic.query}
            type="button"
            ...
            disabled={disabled}
            className={cn(
              "group flex items-center max-sm:shrink-0 gap-1.5 rounded-full border border-[#dce2dc] bg-white px-3.5 py-2 text-xs font-medium text-[#4f5a52]",
