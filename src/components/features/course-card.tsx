...
export function CourseCard({
  ...
  return (
    <Card hover onClick={onClick} className="group relative flex h-full flex-col overflow-hidden">
      <button
        onClick={(e) => {
          e.stopPropagation();
          onToggleFavorite?.();
      ...
      <CardHeader className="flex-1 pr-11">
        ...
        <div className="mt-3 flex items-center gap-2">
          <span className="text-sm font-medium text-brand-orange">{course.provider}</span>
          {course.isFree && (
      ...
      <CardContent className="space-y-3">
        <p className="line-clamp-3 min-h-[63px] text-sm leading-5 text-brand-gray">{course.description}</p>
