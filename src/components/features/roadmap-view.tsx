...
import { ExternalLink, BookOpen, Video, FileText, Code, Route, Clock3, Sparkles } from 'lucide-react';
  ...
  return (
    <div className="relative pb-7 pl-11 last:pb-0">
      ...
      <div className="rounded-2xl border border-[#dfe5de] bg-white p-5 shadow-[0_8px_25px_rgba(23,33,27,0.045)] sm:p-6">
        <h4 className="mb-2 text-lg font-semibold tracking-tight">{step.title}</h4>
        <p className="mb-5 text-sm leading-6 text-brand-gray">{step.description}</p>
            ...
            {step.resources.map((resource, idx) => (
              <a
                ...
                className={cn(
                  ...
                  'hover:border-brand-orange/30 hover:bg-[#fff5f2] hover:text-brand-orange transition-colors'
                )}
              >
    ...
    return (
      <div className="flex min-h-72 flex-col items-center justify-center gap-4 rounded-2xl border border-[#e0e5df] bg-white py-12">
        ...
        <div className="text-center">
          ...
          <p className="mt-1 text-sm text-brand-gray">This usually takes just a few moments…</p>
        </div>
      </div>
    );
  }
  ...
  if (!roadmap) {
    return (
      <div className="rounded-2xl border border-[#dfe5de] bg-white px-5 py-14 text-center">
        ...
        <h2 className="mt-5 text-lg font-semibold">Your roadmap will appear here</h2>
        <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-brand-gray">
          Enter a topic above and we’ll create a structured path with practical resources for every step.
        </p>
      </div>
    );
  }
