...
const isValid = (k?: string) => k && k.trim().length > 5 && !k.includes('your_');
...
export async function searchCourses(query: string, settings: AISettings): Promise<{ success: boolean; courses?: Course[]; error?: string }> {
  const geminiKey = isValid(settings.geminiApiKey) ? settings.geminiApiKey : process.env.GEMINI_API_KEY;
  const mistralKey = isValid(settings.mistralApiKey) ? settings.mistralApiKey : process.env.MISTRAL_API_KEY;
  ...
  const results = await performWebSearch(`free online courses for ${query}`, settings);
  ...
  if (isValid(mistralKey)) {
    try {
      ...
      const res = await mistral.chat.complete({ model: 'mistral-medium-latest', messages: [{ role: 'user', content: prompt }] });
...
export async function generateRoadmap(topic: string, settings: AISettings): Promise<{ success: boolean; roadmap?: Roadmap; error?: string }> {
  const geminiKey = isValid(settings.geminiApiKey) ? settings.geminiApiKey : process.env.GEMINI_API_KEY;
  const mistralKey = isValid(settings.mistralApiKey) ? settings.mistralApiKey : process.env.MISTRAL_API_KEY;
  ...
  if (settings.primaryProvider !== 'mistral' && isValid(geminiKey)) {
    try {
      ...
      const match = text.match(/\{[\s\S]*\}/);
      if (match) return { success: true, roadmap: { topic, ...JSON.parse(match[0]) } };
    } catch {}
  }

  const results = await performWebSearch(`learning path for ${topic}`, settings);
